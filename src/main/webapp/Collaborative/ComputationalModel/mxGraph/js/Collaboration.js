    ...
    mxUtils.getAll([bundle, STYLE_PATH + '/default.xml'], function (xhr) {
        ...
        if (WebSocket) {
            ...
            if (url.search(reg) != -1) {
                ...
                // wsMxgraph = new WebSocket("ws:localhost:8081/TeamWorking/ComputationalModel/" + groupID);
                wsMxgraph = new WebSocket("ws://172.21.212.7:8056/TeamWorking/ComputationalModel/"+groupID);
            }
        }
        else {
        ...
        wsMxgraph.onopen = function () {
            ...
            window.setInterval(function () {
                if(wsMxgraph!=null){
                    var messageObject = {};
                    messageObject["messageType"] = "Ping";
                    wsMxgraph.send(JSON.stringify(messageObject));
                }
            },20000);
        };
